Valkey {{ spec.version }} in-memory data structure store container image
======================================================

This container image includes Valkey {{ spec.version }} in-memory data structure store for OpenShift and general usage.
...
Valkey {{ spec.version }} available as container, is an advanced key-value store.
...
For this, we will assume that you are using the `rhel8/valkey-{{ spec.version }}` image.
If you want to set only the mandatory environment variables and not store
the database in a host directory, execute the following command:

```
$ podman run -d --name valkey_database -p 6379:6379 rhel8/valkey-{{ spec.version }}
```
...
```
$ podman run -d --name valkey_database -e VALKEY_PASSWORD=strongpassword rhel8/valkey-{{ spec.version }}
```
