...
class TestValkeyApplicationContainer:
    ...
    def test_run_app_test(self, password, test_name):
        ...
        valkey_output = PodmanCLIWrapper.podman_exec_shell_command(
            cid_file_name=cid, cmd=f"{valkey_cmd} ping"
        )
        ...
        assert (
            PodmanCLIWrapper.podman_exec_shell_command(
                cid_file_name=cid, cmd=f"{valkey_cmd} set a 1", return_output=False
            )
            == 0
        )
        # The valkey-cli set value 'b' to 2
        assert (
            PodmanCLIWrapper.podman_exec_shell_command(
                cid_file_name=cid, cmd=f"{valkey_cmd} set b 2", return_output=False
            )
            == 0
        )
        # Checks if value b is set to '2'
        valkey_get_value = PodmanCLIWrapper.podman_exec_shell_command(
            cid_file_name=cid, cmd=f"{valkey_cmd} get b 2>/dev/null"
        ).strip()
    ...
    def test_bind_address(self, bind_address):
        ...
        if bind_address:
            ...
            # Check that VALKEY_CONF really does NOT contain 'bind 127.0.0.1'
            ...
            valkey_return_value = PodmanCLIWrapper.podman_run_command_and_remove(
                cid_file_name=VARS.IMAGE_NAME, cmd=f"valkey-cli -h {cip} -a pass ping"
            ).strip()
            assert "PONG" in valkey_return_value, (
                f"valkey-cli -h {cip} -a pass ping has to return PONG"
            )
